...
module.exports.get = options => new RollingWindowThrottlerDecorator(options);

function RollingWindowThrottlerDecorator(options){
  this.throttler = new RollingWindowThrottler(options)
}

RollingWindowThrottlerDecorator.prototype.tryAcquire = function(key){
  return this.throttler.tryAcquire(key);
};

class RollingWindowThrottler{
  constructor(options) {
    this.max = options.max;
    this.durationWindow = this.calculateDuration(options.durationWindow);
  ...
  tryAcquire(key){
    ...
    this.incrementInvocationCount(invocation);
  ...
  calculateDuration(durationWindow){
